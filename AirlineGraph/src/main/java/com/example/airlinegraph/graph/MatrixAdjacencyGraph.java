...
public class MatrixAdjacencyGraph<E> implements IGraph<E>{
    ...
    public MatrixAdjacencyGraph(boolean isDirected){
    ...
    public Edge<E> dijkstra(E eSource, E eDestination) {
        ...
        return new Edge<>(path, destination.getD());
    ...
    public Map<Pair<E, E>, Edge<E>> floydWarshall() {
        ...
        Map<Pair<E, E>, Edge<E>> paths = new HashMap<>();
        ...
        for(int i = 0; i < n; i++){
            for(int j = 0; j < n; j++){
                ...
                paths.put(new Pair<>(aux.get(i).getElement(), aux.get(j).getElement()),
                        new Edge<>(listPath, distance[i][j]));
